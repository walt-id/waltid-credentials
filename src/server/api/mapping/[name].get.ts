...
export default defineEventHandler(async (event) => {
    ...
    if (matchedContent === undefined) {
        const n = contentQuery
            .filter((content) => {return content.navigation === undefined})
            .map((content) => {return content.title})
    ...
    const codeblocksAndTitles = matchedContent.body.children
        .filter((elem) => {return (elem.tag === "pre" && elem.props?.language === "json") || (elem.tag === "h2")})

    let mappingInTitle = false
    for (const codeblockOrTitle of codeblocksAndTitles) {
        console.log("ITERATING: ", codeblockOrTitle)

        if (codeblockOrTitle.tag === "h2" && codeblockOrTitle?.children?.find((c) => c.type === "text")?.value?.toLowerCase()?.includes("mapping") === true) {
            mappingInTitle = true
            continue
        }
